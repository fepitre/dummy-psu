...
long dummy_psu_ioctl(struct file *filep, unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_PSU_ADD_PSP:
		if (filep->private_data) {
			...
			if (copy_from_user(&psp, (void __user *)arg, sizeof(char[MAX_KEYLENGTH])) != 0)
				return -EFAULT;
			...
			pr_debug("IOCTL_PSU_ADD_PSP: %s: Adding power_supply_property %s (%d)",
	...
	case IOCTL_PSU_ADD_USB_TYPE:
		if (filep->private_data) {
			...
			if (copy_from_user(&usbtype, (void __user *)arg, sizeof(char[MAX_KEYLENGTH])) != 0)
				return -EFAULT;
			parsed_usbtype = strtoenum(usbtype);
			if ((parsed_usbtype >= MAX_PSU_UBS_TYPES) || (parsed_usbtype < 0)) {
				pr_err("IOCTL_PSU_ADD_USB_TYPE: %d: Invalid USB type.\n", parsed_usbtype);
				return -EFAULT;
			}
			pr_debug("IOCTL_PSU_ADD_USB_TYPE: %s: Adding USB type %d", pobj->dev_name, parsed_usbtype);
	...
	case IOCTL_PSU_ADD_SUPPLIED_TO:
		if (filep->private_data) {
			...
			if (copy_from_user(&supplied_to, (void __user *)arg, sizeof(char[MAX_KEYLENGTH])) != 0)
				return -EFAULT;
	...
	case IOCTL_PSU_UPDATE_PROPVAL:
		...
		if (filep->private_data) {
			pobj = filep->private_data;
			if (pobj->psy) {
				if (copy_from_user(&propval, (void __user *)arg, sizeof(struct ioctl_pspval)) != 0)
					return -EFAULT;
				parsed_psp = strtoenum(propval.psp);
				for (i = 0; i <= pobj->num_properties; i++) {
					if (parsed_psp == pobj->props[i]) {
						if (power_supply_is_str_property(parsed_psp)) {
							strncpy(pobj->psp_val[parsed_psp].strval, propval.val, MAX_KEYLENGTH);
							pobj->psp_val[parsed_psp].strval[MAX_KEYLENGTH - 1] = '\0';
						} else if (parsed_psp == POWER_SUPPLY_PROP_STATUS) {
							pobj->psp_val[parsed_psp].intval = map_get_value(map_status, propval.val);
						} else if (parsed_psp == POWER_SUPPLY_PROP_HEALTH) {
							pobj->psp_val[parsed_psp].intval = map_get_value(map_health, propval.val);
						} else if (parsed_psp == POWER_SUPPLY_PROP_TECHNOLOGY) {
							pobj->psp_val[parsed_psp].intval = map_get_value(map_technology, propval.val);
						} else if (parsed_psp == POWER_SUPPLY_PROP_CAPACITY_LEVEL) {
							pobj->psp_val[parsed_psp].intval = map_get_value(map_capacity, propval.val);
						} else {
							if (kstrtoint(propval.val, 10, &pobj->psp_val[parsed_psp].intval) != 0) {
								pr_debug("IOCTL_PSU_UPDATE_PROPVAL: %s: %s: Failed to get property value\n", pobj->dev_name, propval.psp);
								return -EINVAL;
							}
						}
